...
// Decade grid + copy
const decadeButtons = Array.from(document.querySelectorAll(".genre-btn"));
...
function decadeLabel(decade) {
  if (decade === "1970s+1980s") return "1970s + 1980s";
  return decade;
}
...
decadeButtons.forEach(btn => {
  btn.addEventListener("click", () => {
    const decade = btn.dataset.genre; // Note: HTML still uses data-genre attribute
    if (decade === "Random") {
      const decadesWithSongs = DISPLAY_DECADES.filter(d => songs.some(s => decadeFromYear(s.year) === d));
      if (decadesWithSongs.length === 0) {
        renderResult(null, "No songs yet. Add or import some!");
        return;
      }
      pickAndArchiveForDecade(randomItem(decadesWithSongs));
    } else {
...
async function pickAndArchiveForDecade(decade) {
  ...
  if (pool.length === 0) {
    renderResult(null, `No songs in ${decadeLabel(decade)} yet. Add or import some!`);
    return;
  }
...
function applyFilters(list, scope) {
  ...
  return list.filter(s => {
    const matchesDecade = !decade || decadeFromYear(s.year) === decade;
    const matchesQuery =
      !q ||
  ...
  if (!song) {
    resultEl.innerHTML =
      '<div class="meta">' +
        ...
        '<div class="subtitle">Use the decade grid to pick a song.</div>' +
      '</div>';
    return;
  }
...
function renderCounts(list) {
  ...
  const badges = DISPLAY_DECADES
    .map(function(d) {
      return '<span class="count-badge">' + decadeLabel(d) + ": " + counts[d] + "</span>";
    })
    .join(" ");
